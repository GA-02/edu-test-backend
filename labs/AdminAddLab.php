...
$maxNumberLab = mysqli_query($link, "SELECT MAX(`EndNumber`)+1 FROM `labs` WHERE `IDStatus`=2");
$maxNumberLab = mysqli_fetch_array($maxNumberLab)[0];
 
mysqli_query($link, "INSERT INTO `labs`(`IDLab`, `StartNumber`, `EndNumber`, `Theme`, `Goal`, `Equipment`, `Content`, `IDStatus`) VALUES (NULL, " . $maxNumberLab . ", " . $maxNumberLab . ", '', '', '', '', 1)");
$idLab = mysqli_insert_id($link);
echo $idLab;
