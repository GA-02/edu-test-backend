...
$chapters = mysqli_query($link, "SELECT `IDChapter`, `Name`, `Description`, `SerialNumber` FROM `chapters` ORDER BY `SerialNumber`");

while ($chapter = mysqli_fetch_array($chapters)) {
    $answer[] = ['idChapter' => $chapter[0], 'nameChapter' => htmlspecialchars_decode($chapter[1], ENT_QUOTES), 'descriptionChapter' => htmlspecialchars_decode($chapter[2], ENT_QUOTES), 'serialNumber' => $chapter[3]];
}
