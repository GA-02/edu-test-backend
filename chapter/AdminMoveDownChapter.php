...
$maxSerialNumber = mysqli_query($link, "SELECT MAX(`SerialNumber`) FROM `chapters`");
$maxSerialNumber = mysqli_fetch_array($maxSerialNumber)[0];
if ($serialNumberUpdatedChapter >= $maxSerialNumber) {
    echo 'Глава последняя в списке';
    exit;
}
...
echo 'Глава успешно перемещена на одну главу ближе к концу списка.';
